...
if len(sys.argv) < 2 or (sys.argv[1] != 'train' and sys.argv[1] != 'test' and sys.argv[1] != 'record'):
    raise ValueError('usage: python data/face_landmark_detection.py [train|test]')

phase = sys.argv[1]
dataset_path = 'datasets/'
faces_folder_path = os.path.join(dataset_path, phase + '_images/')
...
select_folder = '*'
if(len(sys.argv) == 3):
	select_folder = sys.argv[2]

img_paths = sorted(glob.glob(faces_folder_path + select_folder))
for i in range(len(img_paths)):
    ...
    for img_name in sorted(glob.glob(os.path.join(f, '*.png'))):
